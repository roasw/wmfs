...
let
  ...
  referenceLocal = pkgs.python3Packages.buildPythonPackage {
...
in
{
  ...
  reference-local = referenceLocal;

  reference-python-worker = pkgs.python3Packages.buildPythonApplication {
    pname = "wmfs-reference-worker";
    inherit version;
    pyproject = true;
    src = source + "/plugins/reference-worker";
    SETUPTOOLS_SCM_PRETEND_VERSION_FOR_WMFS_REFERENCE_WORKER = version;
    ...
    pythonImportsCheck = [ "wmfs_reference_worker" ];

    postInstall = ''
      mkdir -p "$out/share/wmfs/plugins/reference"
      cp -R ${referenceLocal}/share/wmfs/plugins/reference/. \
        "$out/share/wmfs/plugins/reference/"
