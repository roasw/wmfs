...
- `benchmark-master.json`: schema-12 bundled Python/native and isolated report.
...
Schema 9, schema 11, and schema 12 primary backend samples stop at backend return. Result
...
the former local/bundled/isolated schema. `benchmark-master.json` compares
bundled Python, bundled native, and isolated execution and includes
...
| master   | Ring round trip |       0.068 |    0.157 |

The native ring dispatcher is 26% faster at the median than the freshly
...
`0.1.0` and 813 calls/s on master; with reusable `out=`, it is 2,848 calls/s and
954 calls/s respectively. Mapping, allocation, reclamation, and Python
