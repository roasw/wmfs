...
TORCH_LIBRARY(wmfs_reference, module) {
    ...
    module.def("add_scalar.out(Tensor a, float value, *, Tensor(a!) out) -> "
               "Tensor(a!)");
    module.def("nonzero(Tensor a, int order=0) -> Tensor");
    module.def("nonzero.out(Tensor a, int order=0, *, Tensor(a!) out) -> "
               "Tensor(a!)");
}
...
TORCH_LIBRARY_IMPL(wmfs_reference, CompositeImplicitAutograd, module) {
    ...
    module.impl("nonzero", TORCH_FN(wmfs::reference::nonzero));
}
...
TORCH_LIBRARY_IMPL(wmfs_reference, CPU, module) {
    ...
    module.impl("nonzero.out", TORCH_FN(wmfs::reference::nonzero_out));
}
