...
def test_generated_headers_and_stub_compile_as_cpp11(tmp_path: Path) -> None:
    ...
    source.write_text(
        ...
        "std::int32_t initialize(wmfs_json_view_v1, logger, wmfs_error_buffer_v1 *) "
        "{ return WMFS_STATUS_OK; }\n"
        "void shutdown() {}\n"
        "} }\n"
        "int main() {\n"
        "  const wmfs_plugin_api_v1 *api = wmfs_reference_plugin_get_api(1);\n"
        "  wmfs::reference::logger null_logger; "
        "if (null_logger.enabled(WMFS_LOG_INFO)) return 5; "
        'null_logger.info("ignored", 7);\n'
