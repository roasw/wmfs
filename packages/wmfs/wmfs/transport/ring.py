"""Runtime-owned stable Linux shared-ring record codec."""
...
if _ATOMIC_LIBRARY is None:
    raise RuntimeError("wmfs.transport.ring requires libatomic")
...
def _load_u64(mapping: mmap.mmap, offset: int) -> int:
    cell = c_uint64.from_buffer(mapping, offset)
    try:
...
def _load_u32(mapping: mmap.mmap, offset: int) -> int:
    cell = c_uint32.from_buffer(mapping, offset)
    try:
...
def encode(record: Record) -> bytes:
    ...
    encoded_type = record.error_type.encode()
    encoded_message = record.error_message.encode()
    error_type = encoded_type[:64]
    error_message = encoded_message[:1024]
    error_flags = (1 if len(encoded_type) > 64 else 0) | (
        2 if len(encoded_message) > 1024 else 0
    )
...
def decode(data: bytes, generation: int) -> Record:
    ...
    (
        ...
        reserved0,
        reserved1,
    ) = _RECORD_HEADER.unpack_from(data)
    ...
    for index in range(tensor_count):
        ...
        (
            ...
            parameter,
            reserved,
        ) = _TENSOR_HEAD.unpack_from(data, offset)
    ...
    type_length, message_length, error_flags, error_reserved = _ERROR_HEAD.unpack_from(
        data, _ERROR_OFFSET
    )
    if error_reserved or error_flags & ~3 or type_length > 64 or message_length > 1024:
        raise RingError("invalid ring error descriptor")
    if any(data[_ERROR_OFFSET + 16 + type_length : _ERROR_OFFSET + 80]) or any(
        data[_ERROR_OFFSET + 80 + message_length : _TAIL]
    ):
    ...
    return Record(
        ...
        data[_ERROR_OFFSET + 16 : _ERROR_OFFSET + 16 + type_length].decode(),
        data[_ERROR_OFFSET + 80 : _ERROR_OFFSET + 80 + message_length].decode(),
    )
    ...
    def push_profiled(self, record: Record, timeout: float | None = None) -> int:
        ...
        with self._lock:
            while True:
                ...
                self._wait(self.space_fd, deadline)
                wait_ns += perf_counter_ns() - started
